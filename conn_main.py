...
from settings import KafkaParams, ConnGroupId, ConnTopic, NidsAlertTopic, SparkLogLevel
...
def send_partition(iter):
    ...
    conn_check = Engine(rule_type='CONN')
    for record in iter:
        try:
            if conn_check.check_line(record):
                for check_result in conn_check.check_line(record):
                    if check_result[0]:
...
sc_conf = SparkConf()
sc_conf.setAppName('sec-' + ConnTopic)
sc = SparkContext(conf=sc_conf)
sc.setLogLevel(SparkLogLevel)
ssc = StreamingContext(sc, 5)
offset_ranges = get_offset_ranges(ConnTopic)
