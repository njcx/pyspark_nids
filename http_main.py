...
from settings import KafkaParams, HTTPGroupId, HTTPTopic, NidsAlertTopic, SparkLogLevel
...
def send_partition(iter):
    ...
    http_check = Engine(rule_type='HTTP')
    for record in iter:
        try:
            if http_check.check_line(record):
                for check_result in http_check.check_line(record):
                    if check_result[0]:
...
sc_conf = SparkConf()
sc_conf.setAppName('sec-' + HTTPTopic)
sc = SparkContext(conf=sc_conf)
sc.setLogLevel(SparkLogLevel)
ssc = StreamingContext(sc, 5)
offset_ranges = get_offset_ranges(HTTPTopic)
