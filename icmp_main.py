...
from settings import KafkaParams, ICMPGroupId, ICMPTopic, NidsAlertTopic, SparkLogLevel
...
def send_partition(iter):
    ...
    icmp_check = Engine(rule_type='ICMP')
    for record in iter:
        try:
            if icmp_check.check_line(record):
                for check_result in icmp_check.check_line(record):
                    if check_result[0]:
...
sc_conf = SparkConf()
sc_conf.setAppName('sec-' + ICMPTopic)
sc = SparkContext(conf=sc_conf)
sc.setLogLevel(SparkLogLevel)
ssc = StreamingContext(sc, 5)
offset_ranges = get_offset_ranges(ICMPTopic)
msg_stream = KafkaUtils.createDirectStream(ssc, [ICMPTopic],
                                           kafkaParams=dict(KafkaParams, **{"group.id": ICMPGroupId}),
